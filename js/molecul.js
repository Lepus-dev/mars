...
(function() {
    ...
    function initHeader() {
        ...
        for(var i in points) {
            var c = new Circle(points[i], 2+Math.random()*2, '#000');
            points[i].circle = c;
        }
    }
    ...
    function animate() {
    	if (width < 1400) {
    		var firstNumber = 790;
    		var secondNumber = 5190;
    		var thirdNumber = 70090;
    	}
    	if (width > 1400) {
    		var firstNumber = 7900;
    		var secondNumber = 51000;
    		var thirdNumber = 700000;
    	}
        if(animateHeader) {
            ctx.clearRect(0,0,width,height);
            for(var i in points) {
                // detect points in range
                if(Math.abs(getDistance(target, points[i])) < firstNumber) {
                    points[i].active = 0.3;
                    points[i].circle.active = 0.6;
                } else if(Math.abs(getDistance(target, points[i])) < secondNumber) {
                    points[i].active = 0.1;
                    points[i].circle.active = 0.3;
                } else if(Math.abs(getDistance(target, points[i])) < thirdNumber) {
